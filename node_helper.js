...
module.exports = NodeHelper.create({
    ...
    activateMonitor: function () {
        ...
        let alwaysOffTrigger = this.alwaysOff && (this.alwaysOff.value === this.config.alwaysOffState)
        if (alwaysOffTrigger) {
            return;
        }
        // If relays are being used in place of HDMI
        if (this.config.relayPin !== false) {
            this.relay.set(this.config.relayState);
        }
    ...
    deactivateMonitor: function () {
        ...
        let alwaysOnTrigger = this.alwaysOn && (this.alwaysOn.value === this.config.alwaysOnState)
        let alwaysOffTrigger = this.alwaysOff && (this.alwaysOff.value === this.config.alwaysOffState)
        ...
        // If relays are being used in place of HDMI
        if (this.config.relayPin !== false) {
            this.relay.set((this.config.relayState + 1) % 2);
        }
        else if (this.config.relayPin === false) {
    ...
    socketNotificationReceived: function (notification, payload) {
        if (notification === 'CONFIG' && this.started == false) {
            ...
            if (this.config.relayPin) {
                this.relay = gpio.export(this.config.relayPin, {
                    direction: gpio.DIRECTION.OUT,
                    ready: function() {
                        self.relay.set(self.config.relayState);
                        exec("/usr/bin/vcgencmd display_power 1", null);
                        if (self.config.supportCEC)
            ...
            if (this.config.alwaysOnPin) {
                this.alwaysOn = gpio.export(this.config.alwaysOnPin, {
                    direction: gpio.DIRECTION.IN,
                    ready: function() {
                        const alwaysOnState = self.config.alwaysOnState;
                        self.alwaysOn.on("change", function(val) {
                            if (val === alwaysOnState) {
            ...
            if (this.config.alwaysOffPin) {
                this.alwaysOff = gpio.export(this.config.alwaysOffPin, {
                    direction: gpio.DIRECTION.IN,
                    ready: function() {
                        const alwaysOffState = self.config.alwaysOffState;
                        self.alwaysOff.on("change", function(val) {
                            if (val === alwaysOffState) {
                                self.sendSocketNotification('ALWAYS_OFF', true);
                                self.deactivateMonitor();
                            } else if (val === (alwaysOffState + 1) % 2) {
